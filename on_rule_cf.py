...
# Kiểm tra tất cả các rule trong Cloudflare
def check_all_rules(api_key, email, zone_id):
    ...
    if response.status_code == 200:
        ...
        for rule in rules:
            print(f"Rule ID: {rule.get('id')}, Description: {rule.get('description')}, Mode: {rule.get('mode')}")
    else:
        print(f"Failed to retrieve rules: {response.text}")
...
check_all_rules(cloudflare_api_key, cloudflare_email, zone_id)
